...
//importante configurar los interceptores en un archivo config 
    ...
    @Override //El request podemos recibir datos de la petición y el resonse es para que nosostros mandemos mensajes, el handler es el controlador
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        request.setAttribute("start", start);//Guardamos el start en el request porque es lo que se comparte entre el pre y el post y así ver cuanto se tarda

        Random random = new Random();
        int delay = random.nextInt(500); //Rango de valor entre 0 y 500
        Thread.sleep(delay); 

        Map<String, String> json = new HashMap<>(); //Estas lineas de código es por si es falso, que retorne un json de error
